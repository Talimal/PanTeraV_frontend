...
const App = (props) => {
  ...
  const findTirpFromProps = ()=>{
    let tirpFromProps = JSON.parse(sessionStorage["CurrentTirp"]);
    let symbolsNamesArr = tirpFromProps["_TIRP__symbols"];
    let relationsArr = tirpFromProps["_TIRP__rel"];

    for(var i=0;i<tirpsList.current.length;i++){
      if(compareArrays(tirpsList.current[i].getSymbolsNames(),symbolsNamesArr)
        && compareArrays(tirpsList.current[i].getReationsNames(),relationsArr)){
          return tirpsList.current[i];
      }
    }
    return null;
  }
  ...
  const findSymbolFromProps = ()=>{
    ...
    let tirp = findTirpFromProps();
    let symbolIndex = tirp.getSymbolsNames().indexOf(symbolFromProps);
    return tirp.getSymbols()[symbolIndex];
  }
  ...
  const createRelationsJson = (tirpsJson)=>{
    ...
    for(var index in tirpsJson){
      ...
      for(var tirpIndex in tirpsJson[index]){
        ...
        const symbolsNames = tirpsJson[index][tirpIndex]['symbols_names'];
        ...
        const mean_of_first_interval = tirpsJson[index][tirpIndex]['mean_of_first_interval'];
        const mean_offset_from_first_symbol = tirpsJson[index][tirpIndex]['mean_offset_from_first_symbol'];

        const newTirp = new TIRP(size,symbols,symbolsNames,relations,numSupEnt,meanHorSup,
