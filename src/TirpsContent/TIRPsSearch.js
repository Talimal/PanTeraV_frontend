...
class TIRPsSearch extends Component {
	constructor(props) {
		...
		this.state = {
			parameters: {
				...
				minHSCls0: 1,
				maxHSCls0: 100,
				minVSCls0: parseFloat(localStorage.min_ver_support) * 100,
				maxVSCls0: 100,

				minHSCls1: 1,
				maxHSCls1: 100,
				minVSCls1: parseFloat(localStorage.min_ver_support) * 100,
				maxVSCls1: 100,
			},
	...
	async getAllTirps() {
		...
		const allTirpsEntries = allTirps.map((tirp) => [tirp['_TIRP__symbols'].join(','), tirp]);
	...
	handleOnSelect(newSelected) {
		const symbolsNumbers = newSelected[this.props.isPredictive ? 7 : 4].split(/[(-]+/);
		const cleanedSymbolsNumber = symbolsNumbers.slice(1, symbolsNumbers.length - 1);
		const symbolNames = cleanedSymbolsNumber.map(
			(symbolNumber) => this.state.dictionary_states[symbolNumber]
		);
		...
		this.setState({
			...
			canExplore: true,
			chosenTIRP: this.state.allTirps[symbolString],
		});
	}
