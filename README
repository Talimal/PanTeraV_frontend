...
You can install the packages using the command - "npm -i package.json"
* run react client side by executing the command: "npm start" (from the same directory where package.json is).

Before experiencing with the system, here is an explanation regarding all the files in the project:
* src - a folder that contains all the code for PanTeraV's client side.  
App.js is the main file, that connects to the backend and requests for data to present.  
Then, all the TreeView screen (where the TIRPs and STIs are presented) is controlled by ExploreTreeController.  
* TreeView_master is the main folder, that controls the transitions between TIRPs, branches in the enumeration tree and STIs visualizations.  
* Every .js file is another component, of the main exploration screen, the majority of them are visualization components, like graphs, and custom visual views.  
* TIRPsContent is a folder that contains small components, related to TIRPs presentation.  
* DataStructures is a small folder, with a representation for RelationVector (temporal relations between every pair of STIs in a TIRP) and the TIRP structure.  
* ServiceWorker.js defines the communication with the server's side.  
